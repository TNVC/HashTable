...
В данной главе мы будем соривноваться с опцией O3. Мы посмотрим, как она оптимизирует код, и попробуем оптимизировать код лучше.
...
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%230_start/0.png?raw=true)
 ...
 Также представем результаты для опции O3, для будущего сравнения.
...
(Абсолютный прирост - прирост производительности относительно начальной замерки)

(Относительный прирост - прирост производительности относительно предыдущей замерки)
...
Так как теперь размер ключа - фиксированное значение, то можно попробовать заменить на их апаратно поддерживаемые. Среди таких есть ранее нам знакомый CRC32. Хоть у него и более худшая дисперсия, но тут вместо цикла по всем 32 элементам строки, ножно использовать четыре инструкции обрабатывающие по 8 символов за один раз.

Хоть раньше и было зафиксированно, что ассемблерная вставка дает лучший прирост производительности при O0, чем ассемблерная реализация, но для экспиремента было принято решение использовать именно ассемблерную реализацию.

Для справки, также в самом конце была реализована и ассемблерная вставка и было замечено, что при O0 количество инструкций в `GetHash` было увеличено на 1M, но общее количество уменьшилось на 32M. При этом при опции O3 общее количество инструкций увеличилось на 2M. Так что данное решение было правильным.
