...
Для тестированния модели была выбрана GNU реализация хеш-функции из-за наилучшего покозателя дисперсии.
...
<details>
<summary> O0 </summary>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_start/0.png?raw=true)

</details>

 Также представем результаты для опции O3, для будущего мравнения.

<details>
<summary> O3 </summary>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_start/3.png?raw=true)

</details>

Как видно больше всего инструкций приходится на `GetHash` и среднее количество инструкций на один вызов состовляет 170,2.

Можно предположить, что ассемблерный код, сгенерированный компилятором не является эффективным.
Из решения данной проблемы можно либо реализовать свою ассемблерную версию или исрользовать ассемблерную вставку.

Было предположение, что ассемблерная вставка будет медленей из-за формирования стекового кадра, но для интереса были проверенны все версии.

#### Оптимизация №1: Замена реализации хеш-функции.

<details>
<summary> Asm </summary>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_asm/asm.png?raw=true)

</details>

<details>
<summary> Inline asm with O0 </summary>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_asm/inlineO0.png?raw=true)

</details>

<details>
<summary> Inline asm with O1 </summary>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_asm/inlineO1.png?raw=true)
...
Также посчитаем коэффициент ассемблерной производительности:
- (для Asm): 91,5
- (для Inline asm with O0): 98,5
- (для Inline asm with O1): 99,2

Код ассемблерной вставки и ассемблерной реализации индентичен.
На удивление, версия с ассемблерной вставкой лучше, чем с ассемблерной реализацией. В случае с опцией O1 компилятор единственное отличие ассемблерной вставки и ассемблерной реализациии - наличие инструкции `endbr64`, и как мы видим общее количество инструкций в данных версий функции `GetHash` сократилось на 8M инструкций, а также общее количество инструкций было меньше на 50M инструкций. Но самое интересное, что версия ассемблерной вставки с опцией O0, где кроме `endbr64` был и стековый кадр, но и при этом `GetHash` содержало на 2M, а общее количество уменьшилось на 44M.

Дальше была принята версии Inline asm with O1.

Теперь можно проанализировать полученные результаты. Первое место занимает функция `Get`. Так как в самой функции нет явных проблемных мест, то для оптимизации можно уменьшить нагрузку на функцию путем увелечения размера до 112.111 ячеек.
...
<details>
![alt text](https://github.com/TNVC/HashTable/blob/master/callgrind/%231_newSize/0.png?raw=true)
</details>
...
Первые два пунка занимают функции, которые уже были оптимизированы, поэтому они будут пропущены.

В таком случае стоит расмотреть `__strcmp_avx2`. Данная функция присутсвует в рейтинге из-за факта неравномерности длины строк, также по данной причине в списке видна и функция `__strchr_avx2`.
Данную проблему можно решить путем заменны типа данных. Вместо строк произвольной длины с терминурующим символом строки фиксированной длины без терминирующего символа. В данном случае в поиски при сравнении ключей можно будет использовать `_mm256_testc_si256`, что решит вопрос использования `__strcmp_avx2`. Но для решение вопроса использования `__strchr_avx2` можно нормировать базу данных - перевести их из текстового в бинарного. Перевод таблицы будет происходить следующим образом - строки меньше 32 символов до 32 путем добавления нулевых символов, а строки длиней - путем отсекания.
...
Так как теперь размер ключа - фиксированное значение, то можно попробовать заменить на их опаратно поддерживаемые. Среди таких есть ранее нам знакомый CRC32. Хоть у него и более худшая дисперсия, но тут вместо цикла по всем 32 элементам строки, ножно использовать четыре инструкции обрабатывающие по 8 символов за один раз.

Хоть раньше и было зафиксированно, что ассемблерная вставка дает лучший прирост производительности при O0, чем ассемблерная реализация, но для экспиремента было принято решение использовать именно ассемблерную реализацию. Для справки также в саммом конце была реализована и ассемблерная вставка и было замечено, что при O0 количество инструкций в `GetHash` было увеличено на 1M, но общее количество уменьшилось на 32M. При этом при опции O3 общее количество инструкций увеличилось на 2M. Так что данное решение было правильным.
...
- Для абсолютного с учетом ассемблерной вставки: 151.4
- Для абсолютного без учетом ассемблерной вставки: 397.5
- Для относительного: 181.25

Пропустим `Get`, по уже ранее описанным причинам. Следующая функция `AreKeysEqual` оптимизации не подлежит, так как сводится к вызову встраивоймой функции `_mm256_testc_si256` и сама она объявлена как inline, но из-за опции O0 вставка небыла произведена.

Поэтому расмотрим `_int_malloc`. Также просмотря список, можно заметить и другие функции работы с динамической памяти. Проанализируем текущий принцип использования динамической памяти. На данный момент память представляет собой много разбросанных малых кусков, соединенных принципом цепочки. Так как время выполнения `calloc` линейно зависит от количества выделенной памяти(во всей программе), а также и `DestroyHashTable`(для решения коллизий списками) линейно зависит от количества элементов в таблице, то данный фактор будет сильно замедлять работу при большой базе данных и/или частом использовании пользователем malloc/calloc. Было решено использовать свои функции работы с динамической памятью, которые будут работать в заранее заказанной большой области памяти, которая будет определяться по размеру таблицы. Конечно наша реализация будет совподать с стандартной, но с отличием в том, что вме выделяемые области памяти имеют один размер. Также очистка динамической памяти будет сведена к одному вызову `free`.

Но в данном случае есть важный недостаток - заполнение буфера.
...
Далее смотря на таблицу явно не видно, что можно оптимизировать. Но всегда можно посмотреть на код, генерируемый компилятором при O3.
...
В данном случае тоже не видно, что можно оптимизировать.

Поэтому было принято решение прекратить серию оптимизаций и посчитать результаты.

#### Итоги главы

Будем сравнивать изначальные характеристики с последним тестированием, но так как в нем было представлены версии с опциями O0 и O3, то и сравнивать будем, как с первой так и со второй версиями.

Прирост производительности версий с разными оптимизациями:
| Версии для сравнения | Прирост     | Коэф. ассемблерного прироста |
| -------------------- | ----------- | ---------------------------- |
| V.0(O0):V.0(O3)      | 1,64        |   0,0                        |
| V.0(O0):V.5(O0)      | 4,03        | 191,9                        |
| V.0(O0):V.5(O3)      | 9,60        | 457,1                        |
| V.0(O3):V.5(O3)      | 5,85        | 278,6                        |
